...
public class NumberGroundingsCalculator {
	...
	public long countNumberOfNonTrivialGroundings(Clause cl, Literal eg) {
		...
		if (isOnlyInHead(cl, eg)) {
			...
			//Total gounding counting----NR for MLN
			totalCount = 0.0;
...
//				System.out.println(cl.getDefiniteClauseBody()+"---------"+RunBoostedRLR.gdb);
//				
				totalCount = RunBoostedRLR.gdb.totalCount(cl.getDefiniteClauseHead().getArguments().toString().replace("[", "").replace("]", "").replace(", ", ","),
			...
			double cnt = RunBoostedRLR.gdb.getApproxCount(cl.getDefiniteClauseHead().getArguments().toString().replace("[", "").replace("]", "").replace(", ", ","),
					eg.getArguments().toString().replace("[", "").replace("]", "").replace(", ", ","), preds, bitrep);
