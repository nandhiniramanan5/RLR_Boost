...
public class RegressionMLNModel extends RegressionTree {
	...
	public static RegressionValueOrVector val2;
	public static RegressionValueOrVector val0;
	...
	public static RegressionValueOrVector getval0(Clause clause, Example ex) {
		return val0;
	}
...
//		Term y = head.getArgument(head.numberArgs()-1);
		Term y = head.getArgument(head.numberArgs()-3);
		Term y2 = head.getArgument(head.numberArgs()-2);
		Term y0 = head.getArgument(head.numberArgs()-1);
		...
		val2 = BoostingUtils.getRegressionValueOrVectorFromTerm(y2);
		val0 = BoostingUtils.getRegressionValueOrVectorFromTerm(y0);
		head.removeArgument(y);
		head.removeArgument(y2);
		head.removeArgument(y0);
		...
		if (inCache(cl, ex)) {
			...
			total = NumberGroundingsCalculator.totalCount;
