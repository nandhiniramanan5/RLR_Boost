					...
					if (parent == null || !parent.posExampleAlreadyExcluded(posEx)) { // Don't look at THIS node or we'll have an infinite loop.
						...
						if (theILPtask.regressionTask && !theILPtask.oneClassTask) {
							...
							if (!theILPtask.MLNRegressionTask) { // Example counting for MLN tasks later
								...
								{
								int clSize = this.getClause().getDefiniteClauseBody().size();
								String bitrep = "";
								...
								List<Literal> preds = this.getClause().getDefiniteClauseBody();
//								System.out.println(caller.getClause().getDefiniteClauseBody()+"---------"+RunBoostedRLR.gdb);
								
								
								
								totalCount = RunBoostedRLR.gdb.totalCount(this.getClause().getDefiniteClauseHead().getArguments().toString().replace("[", "").replace("]", "").replace(", ", ","),
...
//								Utils.println("adding "  + posEx + ":" + this.getClause() + ":" + total);
								cachedLocalRegressionInfoHolder.getFalseStats().addFailureExample(posEx, 1,neg, posEx.getWeightOnExample());
							}
...
//							Utils.println("adding "  + posEx + ":" + this.getClause() + ":" + total);
							...
							cachedLocalRegressionInfoHolder.getFalseStats().addFailureExample(posEx, num, neg, fraction*posEx.getWeightOnExample());
						}
					} 
...
//		}
	...
	private double getMLNCost(BranchStats stats) {
		...
		double result = 0;
		if (stats.getsumOfNumGroundingSquared_pos() > 0) {
...
//	}
	...
	*/
	...
	public double MLNRegressionForTrue_2() throws SearchInterrupted {
		...
		//return holder.trueBranchStats.getLambda(((LearnOneClause)task).useProbabilityWeights);
		RegressionInfoHolder holder = getRegressionInfoHolder();
		return holder.meanAtSuccess2();
	}
	public double MLNRegressionForTrue_0() throws SearchInterrupted {
		// TODO(MLNTEST)
		//RegressionNodeInfoHolderForMLN holder = getRegressionNodeInfoHolderForMLN();
		//return holder.trueBranchStats.getLambda(((LearnOneClause)task).useProbabilityWeights);
		RegressionInfoHolder holder = getRegressionInfoHolder();
		return holder.meanAtSuccess0();
	}
