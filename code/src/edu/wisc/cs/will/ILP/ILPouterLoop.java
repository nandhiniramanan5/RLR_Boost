                    ...
                    if (bestNode != null && bestNode != savedBestNode) { // Also need to check to make sure we didn't simply return the previous root when doing tree-structured learning.
						...
						//NR for RLR
                            ...
                            if (learnMLNTheory && !learningTreeStructuredTheory) {
								...
								double reg2 = bestNode.MLNRegressionForTrue_2();
                            	double reg0 = bestNode.MLNRegressionForTrue_0();           
								...
								getStdILPtheory().getClauses().get(len-1).setWeight2OnSentence(reg2);
   ...
   private Term createMLNLeafNodeFromCurrentExamples(Collection<Example> currentExamples, SingleClauseNode node) {
		if (innerLoopTask.regressionTask) {
			...
			for (Example eg : currentExamples) {
				...
				{
				int clSize = node.getClause().getDefiniteClauseBody().size();
				String bitrep = "";
				...
				List<Literal> preds = node.getClause().getDefiniteClauseBody();
//				System.out.println(caller.getClause().getDefiniteClauseBody()+"---------"+RunBoostedRLR.gdb);
				
				
				
				totalCount = RunBoostedRLR.gdb.totalCount(node.getClause().getDefiniteClauseHead().getArguments().toString().replace("[", "").replace("]", "").replace(", ", ","),
						eg.asLiteral().getArguments().toString().replace("[", "").replace("]", "").replace(", ", ","), preds, bitrep);
				}
				Double total = totalCount;
				long  neg = (long) (total-num);
				if (total==0)
				{
					neg=0;
				}
				stats.addNumOutput(num, neg, rex.getOutputValue(), rex.getWeightOnExample(), rex.getProbOfExample().getProbOfBeingTrue());
			}
			...
			*/
			...
			if (learnMLNTheory) {
				...
				for (Clause cl : result.getClauses()) {
					...
					Term leafval2 = innerLoopTask.stringHandler.getNumericConstant(cl.getWeight2OnSentence());
					Term leafval0 = innerLoopTask.stringHandler.getNumericConstant(cl.getWeight0onSentence());
					...
					if (argNames != null) {
						...
						headCopy.addArgument(leafval2, "OutputVarTreeLeaf");
						headCopy.addArgument(leafval0, "OutputVarTreeLeaf");
					} else {
						headCopy.addArgument(leafValue);
						headCopy.addArgument(leafval2);
						headCopy.addArgument(leafval0);			 
					}
					//args.add(leafValue);
