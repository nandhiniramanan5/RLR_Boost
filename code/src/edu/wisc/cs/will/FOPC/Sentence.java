...
public abstract class Sentence extends AllOfFOPC implements Serializable, SLDQuery, SentenceOrTerm {
	...
	//NR for MLN
	protected double weight_2 = defaultWeight;
	protected double weight_0 = defaultWeight;									   
	/**
	...
	//NR for MLN
	public double getWeight2OnSentence() {
		return weight_2;
	}	
	public double getWeight0onSentence() {
		return weight_0;
	}								   
	...
	public Sentence setWeight0OnSentence() { // Set to DEFAULT value if no arguments.
		weight_0 = defaultWeight;
		return this; // Returning this makes it convenient to append '.setWeight' to new's.
	}
	public Sentence setWeight2OnSentence() { // Set to DEFAULT value if no arguments.
		weight_2 = defaultWeight;
		return this; // Returning this makes it convenient to append '.setWeight' to new's.
	}																			  
	public Sentence setWeightOnSentence(double weight) {
		...
		public Sentence setWeight2OnSentence(double weight) {
//		this.weight_2 =weight;
		this.weight_2 = Math.max(minWeight, Math.min(maxWeight, weight)); // Keep in range.
		return this; // Returning this makes it convenient to append '.setWeight' to new's.
	}
	public Sentence setWeight0onSentence(double weight) {
//		this.weight_0 = weight;
		this.weight_0 = Math.max(minWeight, Math.min(maxWeight, weight)); // Keep in range.
		return this; // Returning this makes it convenient to append '.setWeight' to new's.
	}				 
	...
	public List<Clause> convertToClausalForm() {
		...
		if (containsDisjunction) {
			...
			double holdWgt_2 = sentence.getWeight2OnSentence();
			double holdWgt_0 = sentence.getWeight0onSentence();		  
			sentence.setWeightOnSentence(); // Set to the max weight.
			sentence.setWeight0OnSentence();
			sentence.setWeight2OnSentence();		   
			...
			sentence.setWeightOnSentence(holdWgt_2);
	        sentence.setWeightOnSentence(holdWgt_0);			   
			if (debugLevel > 1) { Utils.println("After distributeDisjunctionOverConjunction():\n " + sentence); }
		}
